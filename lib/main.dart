...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _transmitData() async {
    ...
    // final display = await _pinpad.display(DisplayRequest("Tabs Atualizadas!", "TS: ${timestamp.data.timestamp}"));
    // setState(() {
    ...
    //   ..timestamp = TIMESTAMP);
    // setState(() {
    //   _console.writeln("Start get card: ${getCard.status}");
    // });

    final getKey = await _pinpad.getKey();
    setState(() {
      _console.writeln("Start get card: ${getKey.status}");
    });

    // final getCard = await _pinpad.getCard();
    // setState(() {
    //   _console.writeln("Get card: ${getCard.status}");
    // });

    // PinpadResult<GetCardResponse> getCard;

    // setState(() {
    //   _console.writeln();
    // });
    // while (true) {
    //   getCard = await _pinpad.getCard();
    //   if (getCard.status != Status.PP_PROCESSING) break;
    //   setState(() {
    //     _console.write(".");
    //   });
